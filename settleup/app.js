...
function renderSettlementResults(transfers, financials, options = {}) {
  ...
  for (const transfer of transfers) {
    ...
    names.textContent = `${getPersonName(transfer.fromId)} → ${getPersonName(
      transfer.toId
    )}`;
...
function logScenarioResult(label, pass, actual, expected) {
  const formatLine = (transfer) =>
    `${transfer.fromId} → ${transfer.toId} ${formatCurrency(
...
// Manual checklist:
// - Add 3 guests, run all four scenarios → exact outputs.
// - Rename a guest → expenses still attribute correctly.
// - Enter 12,34 and 12.34 both render as €12,34.
// - Refresh → everything is cleared.
